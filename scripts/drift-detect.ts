#!/usr/bin/env tsx
/**
 * Drift detection for Canadian Law MCP.
 *
 * Checks if upstream Justice Laws Website content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Canadian Law MCP — Drift Detection');
  console.log('===================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Canadian-Law-MCP/1.0 drift-detect' },
      });
