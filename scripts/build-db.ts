#!/usr/bin/env tsx
/**
 * Database builder for Canadian Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Canadian Law MCP database...\n');
  ...
  const loadAll = db.transaction(() => {
    for (const file of seedFiles) {
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Canadian statute text',
              );
      ...
      // Deduplicate definitions by term (keep longest definition)
      const defsByTerm = new Map<string, DefinitionSeed>();
      for (const def of seed.definitions ?? []) {
        const key = def.term.toLowerCase().trim();
        const existing = defsByTerm.get(key);
        if (!existing || def.definition.length > existing.definition.length) {
          defsByTerm.set(key, def);
        }
      }
      for (const def of defsByTerm.values()) {
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'CA');
    insertMeta.run('source', 'laws-lois.justice.gc.ca');
    insertMeta.run('licence', 'Open Government Licence - Canada');
  });
  writeMeta();
