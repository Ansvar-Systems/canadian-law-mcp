#!/usr/bin/env tsx
/**
 * Canadian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://laws-lois.justice.gc.ca';
const PORTAL_NAME = 'Justice Laws Website (Canada)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/canadian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Canadian Law MCP — Data Freshness Check');
