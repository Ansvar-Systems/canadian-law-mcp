# Canadian Law MCP Server — Developer Guide
...
Canadian Law MCP server providing bilingual (EN/FR) Canadian federal legislation search via Model Context Protocol. Strategy A deployment (Vercel, bundled SQLite DB).
...
- Canadian citation style: "Section N" or "s. N" (with period after s), range "ss. N-M"
...
1. `scripts/ingest.ts` → fetches from Justice Laws Website → JSON seed files in `data/seed/`
...
- **Justice Laws Website** (laws-lois.justice.gc.ca) — Department of Justice Canada
- **License:** Open Government Licence - Canada
- **Languages:** English (en) and French (fr) are both official
- **Coverage:** All consolidated federal Acts and regulations

## Deployment

- Vercel Strategy A: DB bundled in `data/database.db`, included via `vercel.json` includeFiles
- npm package: `@ansvar/canadian-law-mcp` with bin entry for stdio
