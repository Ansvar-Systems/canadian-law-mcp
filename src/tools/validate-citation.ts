/**
 * validate_citation — Validate a Canadian legal citation against the database.
 */
...
/**
 * Parse a Canadian legal citation.
 * Supports:
 * - "Section 5 PIPEDA" / "s. 5 PIPEDA"
 * - "ss. 5-10 Criminal Code"
 * - "Section 342.1 Criminal Code"
...
function parseCitation(citation: string): { documentRef: string; sectionRef?: string } | null {
  ...
  const rangeFirst = trimmed.match(
    /^ss\.\s*(\d+(?:\.\d+)?)\s*-\s*\d+(?:\.\d+)?\s+(.+)$/i
  );
  if (rangeFirst) {
    return { documentRef: rangeFirst[2].trim(), sectionRef: rangeFirst[1] };
  }

  // "<law> Section N" or "<law>, s. N"
    ...
    if (!provision) {
      return {
        results: {
          ...
          warnings: [...warnings, `Provision "s. ${parsed.sectionRef}" not found in ${doc.title}`],
        },
        _metadata: generateResponseMetadata(db),
      };
    }
