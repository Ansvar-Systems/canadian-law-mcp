/**
 * search_eu_implementations — Search EU directives/regulations with Canadian aligning legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_canadian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as canadian_statute_count
  ...
  if (input.has_canadian_implementation) {
    sql += ' HAVING canadian_statute_count > 0';
  }
